    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if response_data.get('return') == True:
                ...
                print("Fast2SMS Error:", response_data) # Prints to your terminal if it fails
                return JsonResponse({'status': 'error', 'message': 'Failed to send SMS. Check your API limit.'})
                
        except Exception as e:
            print(f"Server SMS Error: {e}")
            return JsonResponse({'status': 'error', 'message': 'Internal server error sending SMS.'})
